...
namespace Application.Features.Tasks.TransportationTask.CreateTransportationTask
{
    ...
    public class CreateTransportationTaskHandler : IRequestHandler<CreateTransportationTask, Unit>
    {
        ...
        public async Task<Unit> Handle(CreateTransportationTask request, CancellationToken cancellationToken)
        {
            ...
            {
                SourceAreaId = request.areaSourceId,
                LinkedWorkArea = 24,
                DestinationAreaId = request.areaDestinationId,
                Support = request.support
            };
            var transport = taskHeader.AddStartingTask(request.areaSourceId, transportTask);
